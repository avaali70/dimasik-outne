...
function generate_secret_key() {
  read -p "Введите секретный ключ (или нажмите Enter для генерации): " user_input

  if [ -z "$user_input" ]; then
    # Если ввод пустой, генерируем новый ключ
    SB_API_PREFIX="$(head -c 16 /dev/urandom | base64 | tr -d '\n')"
    echo "Сгенерированный секретный ключ: $SB_API_PREFIX"
  else
    # Используем введённый пользователем ключ
    SB_API_PREFIX="$user_input"
    echo "Используемый секретный ключ: $SB_API_PREFIX"
  fi
...
function generate_certificate() {
  # Генерируем самоподписанный сертификат и сохраняем его в директории состояния.
  ...
  # Запрашиваем SHA-256 ключ у пользователя
  read -p "Введите SHA-256 ключ (или нажмите Enter для использования автоматически сгенерированного): " sha256_input
  
  if [ -z "$sha256_input" ]; then
    # Генерируем SHA-256 ключ, если ввод пустой
    sha256_input="$(head -c 32 /dev/urandom | base64 | tr -d '\n')"
    echo "Сгенерированный SHA-256 ключ: $sha256_input"
  else
    echo "Используемый SHA-256 ключ: $sha256_input"
  fi

  declare -a openssl_req_flags=(
    -x509 -nodes -days 36500 -newkey rsa:4096
    -subj "/CN=${PUBLIC_HOSTNAME}/SHA256=${sha256_input}"
