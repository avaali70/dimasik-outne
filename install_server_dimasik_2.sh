...
function generate_certificate() {
  read -p "Введите ваш SHA-256 ключ (или нажмите Enter для автоматической генерации): " user_sha256
  ...
  # Если пользователь ввел свой SHA-256 ключ, добавим его в сертификат
  if [ -n "$user_sha256" ]; then
    echo "Используемый SHA-256 ключ: $user_sha256"
    # Здесь вы можете использовать этот ключ для записи в сертификат
    # Но OpenSSL по умолчанию генерирует свой собственный SHA-256
    # Поэтому потребуется кастомизация OpenSSL или использование другого метода
  fi
}
